...
class HabitListRepositoryImpl @Inject constructor(
    ...
    override suspend fun getHabitsByNameAndTypeAndSortASC(
        ...
        return habitDao.getHabitsByNameAndTypeAndSort(nameToFilter, type, SORT_ASCENDING).map { list ->
    ...
    override suspend fun getHabitsByNameAndTypeAndSortDESC(
        ...
        return habitDao.getHabitsByNameAndTypeAndSort(nameToFilter, type, SORT_DESCENDING).map { list ->
            list.map { it.toDomain() }
        }
    }
    ...
    */
            ...
            when (networkStatus) {
                ConnectivityObserver.NetworkStatus.Available -> {
                    val listUnsentHabit = habitDao.getUnsentHabits(
                        isSentToServer = IS_SENT_TO_SERVER_FALSE,
                        type
                    ...
                    for (n in 0..last) {
                        habitDao.updateIdWithIdNetwork(
                            ...
                            listIdFromServer[n],
                            isSentToServer = IS_SENT_TO_SERVER_TRUE
                        )
                    }
                }
    ...
    companion object {
        ...
        private const val IS_SENT_TO_SERVER_FALSE = 0
        private const val IS_SENT_TO_SERVER_TRUE = 1
    }
}